...
class CostEstimateTest < ActiveSupport::TestCase
  ...
  # token shape. The standalone /cost page is gone; this math must survive and
  # stand alone. Expected figures are computed by hand from the formula.
  test "price_with returns the per-call cost from explicit rates, independent of monthly req" do
    ...
    other = CostEstimate.new(profile.with(req: 1))
